...
class PackageDocumentWriter extends PackageDocumentBase {
    ...
    public static void write(final EpubWriter epubWriter, final XmlSerializer serializer, final Book book)
            throws IOException {
        try {
     ...
     */
    private static void writeSpine(final Book book, final EpubWriter epubWriter, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
        serializer.startTag(NAMESPACE_OPF, OPFTags.spine);
        serializer.attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.toc, book.getSpine().getTocResource()
                .getId());
    ...
    private static void writeManifest(final Book book, final EpubWriter epubWriter, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
...
//		writeCoverResources(book, serializer);
    ...
    private static List<Resource> getAllResourcesSortById(final Book book) {
     ...
     * @throws XMLStreamException
     */
    private static void writeItem(final Book book, final Resource resource, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
        if ((resource == null) ||
        ...
        if (StringUtil.isBlank(resource.getId())) {
            log.error("resource id must not be empty (href: " + resource.getHref() + ", mediatype:" + resource
                    .getMediaType() + ")");
            return;
        }
        if (StringUtil.isBlank(resource.getHref())) {
            log.error("resource href must not be empty (id: " + resource.getId() + ", mediatype:" + resource
                    .getMediaType() + ")");
            return;
        }
        ...
        serializer.attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.media_type, resource.getMediaType()
                .getName());
        serializer.endTag(NAMESPACE_OPF, OPFTags.item);
    }
     ...
     * @throws IllegalArgumentException
     */
    private static void writeSpineItems(final Spine spine, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
        for (final SpineReference spineReference : spine.getSpineReferences()) {
            serializer.startTag(NAMESPACE_OPF, OPFTags.itemref);
            serializer
                    .attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.idref, spineReference.getResourceId());
    ...
    private static void writeGuide(final Book book, final EpubWriter epubWriter, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
    ...
    private static void ensureCoverPageGuideReferenceWritten(final Guide guide,
    ...
    private static void writeGuideReference(final GuideReference reference, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
        if (reference == null) {
            return;
        }
