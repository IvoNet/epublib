...
class PackageDocumentMetadataWriter extends PackageDocumentBase {
     ...
     */
    public static void writeMetaData(final Book book, final XmlSerializer serializer)
        ...
        if (book.getCoverImage() != null) { // write the cover image
            ...
            serializer
                    .attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.content, book.getCoverImage().getId());
        ...
        serializer
                .attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.content, Constants.EPUBLIB_GENERATOR_NAME);
        serializer.endTag(NAMESPACE_OPF, OPFTags.meta);

        serializer.endTag(NAMESPACE_OPF, OPFTags.metadata);
    }

    private static void writeSimpleMetdataElements(final String tagName, final List<String> values,
     ...
     * @
     */
    private static void writeIdentifiers(final List<Identifier> identifiers, final XmlSerializer serializer)
            throws IllegalArgumentException, IllegalStateException, IOException {
