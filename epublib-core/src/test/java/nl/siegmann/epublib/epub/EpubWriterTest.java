...
public class EpubWriterTest extends TestCase {
    ...
    public void testBook1() {
        try {
            ...
            // write book to byte[]
            final byte[] bookData = writeBookToByteArray(book);
...
//			fileOutputStream.close();
            ...
            // read book from byte[]
            final Book readBook = new EpubReader().readEpub(new ByteArrayInputStream(bookData));

            // assert book values are correct
            assertEquals(book.getMetadata().getTitles(), readBook.getMetadata().getTitles());
            assertEquals(Identifier.Scheme.ISBN, CollectionUtil.first(readBook.getMetadata().getIdentifiers())
                    .getScheme());
            assertEquals(CollectionUtil.first(book.getMetadata().getIdentifiers()).getValue(), CollectionUtil
                    .first(readBook.getMetadata().getIdentifiers()).getValue());
            assertEquals(CollectionUtil.first(book.getMetadata().getAuthors()), CollectionUtil
                    .first(readBook.getMetadata().getAuthors()));
    ...
    public void testWritingBookWithCoverWithNullId() {
        try {
            ...
            final InputStream is = this.getClass().getResourceAsStream("/book1/cover.png");
            book.setCoverImage(new Resource(is, "cover.png"));
            // Add Chapter 1
            final InputStream is1 = this.getClass().getResourceAsStream("/book1/chapter1.html");
    ...
    private Book createTestBook() throws IOException {
        ...
        book.addSection(chapter2, "Chapter 2 section 1", new Resource(this.getClass()
                                                                              .getResourceAsStream("/book1/chapter2_1"
                                                                                                   + ".html"),
                                                                      "chapter2_1.html"));
    ...
    private byte[] writeBookToByteArray(final Book book) throws IOException {
        final EpubWriter epubWriter = new EpubWriter();
...
//	            book.getMetadata().addIdentifier(new Identifier(Identifier.Scheme.ISBN,
// dto.getIdentifier().getIdentifier()));
//	        else
//	            book.getMetadata().addIdentifier(new Identifier(Identifier.Scheme.UUID,
// dto.getIdentifier().getIdentifier()));
//	       
...
//	            Resource aux = new Resource(HTMLGenerator.generateChapterHtmlStream(dto,chapter),
// "chapter"+chapter.getNumber()+".html");
//	            book.addSection(chapter.getTitle(), aux );
//	        }
//	       
//	        EpubWriter writer = new EpubWriter();
//	        FileOutputStream output = new FileOutputStream(ResourceBundle.getBundle("info.pxdev.pfi.webclient.resources
// .Config").getString("HTML_CHAPTERS")+dto.getId_book()+"\\test.epub");
//	       
//	        try {
