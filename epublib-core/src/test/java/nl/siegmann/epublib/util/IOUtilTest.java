...
public class IOUtilTest extends TestCase {
    ...
    public void testToByteArray1() {
        final byte[] testArray = new byte[Byte.MAX_VALUE - Byte.MIN_VALUE];
        for (int i = Byte.MIN_VALUE; i < Byte.MAX_VALUE; i++) {
            testArray[i - Byte.MIN_VALUE] = (byte) i;
        }
        try {
            final byte[] result = IOUtil.toByteArray(new ByteArrayInputStream(testArray));
    ...
    public void testToByteArray2() {
        final byte[] testArray = new byte[IOUtil.IO_COPY_BUFFER_SIZE + 1];
    ...
    public void testCopyInputStream1() {
        final byte[] testArray = new byte[(IOUtil.IO_COPY_BUFFER_SIZE * 3) + 10];
        final Random random = new Random();
        random.nextBytes(testArray);
        try {
            final ByteArrayOutputStream result = new ByteArrayOutputStream();
            final int copySize = IOUtil.copy(new ByteArrayInputStream(testArray), result);
    ...
    public void testCalcNrRead() {
        ...
        for (int i = 0; i < testData.length; i += 3) {
            final int actualResult = IOUtil.calcNewNrReadSize(testData[i], testData[i + 1]);
            final int expectedResult = testData[i + 2];
