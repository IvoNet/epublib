...
public class SearchIndex {
    ...
    public SearchIndex(final Book book) {
        initBook(book);
    }
    ...
    private static ResourceSearchIndex createResourceSearchIndex(final Resource resource) {
        final String searchContent = getSearchContent(resource);
        if (StringUtils.isBlank(searchContent)) {
            return null;
        }
        final ResourceSearchIndex searchIndex = new ResourceSearchIndex(resource, searchContent);
        return searchIndex;
    }
    ...
    private static List<ResourceSearchIndex> createSearchIndex(final Book book) {
        final List<ResourceSearchIndex> result = new ArrayList<ResourceSearchIndex>();
        if (book == null) {
            return result;
        }
        for (final Resource resource : book.getContents()) {
            final ResourceSearchIndex resourceSearchIndex = createResourceSearchIndex(resource);
    ...
    public SearchResults doSearch(String searchTerm) {
        ...
        for (final ResourceSearchIndex resourceSearchIndex : this.resourceSearchIndexes) {
    ...
    private static String getSearchContent(final Resource resource) {
    ...
    public static String getSearchContent(final Reader content) {
     ...
     */
    private static boolean isHtmlWhitespace(final int c) {
    ...
    public static String cleanText(String text) {
        ...
        final Matcher matcher = WHITESPACE_PATTERN.matcher(text);
    ...
    private static List<SearchResult> doSearch(final String searchTerm, final ResourceSearchIndex resourceSearchIndex) {
        return doSearch(searchTerm, resourceSearchIndex.getContent(), resourceSearchIndex.getResource());
    }

    static List<SearchResult> doSearch(final String searchTerm, final String content, final Resource resource) {
        final List<SearchResult> result = new ArrayList<SearchResult>();
        int findPos = content.indexOf(searchTerm);
        while (findPos >= 0) {
            final SearchResult searchResult = new SearchResult(findPos, searchTerm, resource);
