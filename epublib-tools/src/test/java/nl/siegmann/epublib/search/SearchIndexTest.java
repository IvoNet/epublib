...
public class SearchIndexTest extends TestCase {
    ...
    public void testDoSearch1() {
        try {
            ...
            final SearchIndex searchIndex = new SearchIndex(testBook);
            final SearchResults searchResults = searchIndex.doSearch("a");
    ...
    public void testUnicodeTrim() {
        ...
        for (int i = 0; i < testData.length; i += 2) {
            final String actualText = SearchIndex.unicodeTrim(testData[i]);
    ...
    public void testInContent() {
        ...
        for (int i = 0; i < testData.length; i += 3) {
            final Resource resource = new Resource(((String) testData[i + 1]).getBytes(), MediatypeService.XHTML);
            final String content = SearchIndex.getSearchContent(new StringReader((String) testData[i + 1]));
            final String searchTerm = (String) testData[i];
            final Integer[] expectedResult = (Integer[]) testData[i + 2];
            final List<SearchResult> actualResult = SearchIndex.doSearch(searchTerm, content, resource);
            assertEquals("test " + ((i / 3) + 1), expectedResult.length, actualResult.size());
            for (int j = 0; j < expectedResult.length; j++) {
                final SearchResult searchResult = actualResult.get(j);
                assertEquals(
                        "test " + (i / 3) + ", match " + j, expectedResult[j].intValue(), searchResult.getPagePos());
            }
        }
    }
    ...
    public void testCleanText() {
        ...
        for (int i = 0; i < testData.length; i += 2) {
            final String actualText = SearchIndex.cleanText(testData[i]);
            assertEquals((i / 2) + ": '" + testData[i] + "' => '" + actualText + "' does not match '" + testData[i + 1]
